	...
	actions: {
		...
		async trigger(ctx) {
			try {
				...
				throw new MoleculerError(
					"Failed to initiate activity: " + error.message,
					500
				);
			}
		},
		...
		async getActivities(ctx) {
			const { id } = ctx.params;
			const activities = await IapActivities(
				this.adapter.model.sequelize
			).findAll({ where: { iap_id: id } });
			return activities;
		},
