...
class CheckboxContainer extends React.Component {
  ...
  printActiveCheckboxes = () => {
    // change to applyFilters
...
const MyComponents = {
      ...
      if (myContent != 1) {
        ...
        if (users.length !== 0) {
          ...
          for (var l = 0; l < users.length; l++) {
            // num_learners -> users.length
            var aux_row_id = 0;
            for (var o = 0; o < objectives.length; o++) {
              // props.num_objs -> objectives.length
              ...
              for (var a = 0; a < objectivesAnalytics.length; a++) {
                if (
                  objectivesAnalytics[a].objective_id.toString() == objective_id
                ) {
                  correspondent_analytics_ids.push(
                    objectivesAnalytics[a].analytics_id
                  );
                }
              }
              ...
              for (var s = 0; s < scores.length; s++) {
                if (
                  correspondent_analytics_ids.includes(scores[s].analytics_id)
                ) {
              ...
              for (var i = 0; i < correspondent_analytics.length; i++) {
                for (var j = 0; j < correspondent_scores.length; j++) {
                  if (correspondent_scores[j].user_id == users[l]) {
                    if (
                      correspondent_scores[j].analytics_id ==
                      correspondent_analytics[i].id
                    ) {
                      for (var aa = 0; aa < objectivesAnalytics.length; aa++) {
                        if (
                          objectivesAnalytics[aa].analytics_id ==
                          correspondent_analytics[i].id
                        ) {
                          value +=
                            (correspondent_scores[j].score / 100) *
                            objectivesAnalytics[aa].weight;
                          break;
                        }
                      }
                    }
              ...
              for (var f = 0; f < flags.length; f++) {
                if (correspondent_flags_ids.includes(flags[f].id)) {
                  if (
                    correspondent_analytics_ids.includes(flags[f].analytics_id)
                  ) {
                    // ver se o value tem ou nao uma flag associada
                    if (
                      value >= parseInt(flags[f].min_value) &&
                      value < parseInt(flags[f].max_value)
                    ) {
        ...
        {filteredData_global.map((item) => (
          <tr>
            <td className="tableRow">
              <button onClick={() => updateCondition(item[1])}>
                {item[2]}
              </button>
            </td>
          </tr>
  ...
  Filter_rows: function DatePicker(props) {
    ...
    if (selectedBoxes != 1) {
      // se houver alguma filter box selecionada
      ...
      for (let i = 0; i < selectedBoxes_array.length; i++) {
        ...
        if (selectedBoxes_array[i].includes("below 30")) {
          // algum dos objetivos abaixo de ... (20% por exemplo)
          filtered.push(2); // ID
          filtered.push(0); // min value
        ...
        if (selectedBoxes_array[i].includes("Zero")) {
          // algum dos objetivos abaixo de ... (20% por exemplo)
          filtered.push(3); // ID
          filtered.push(0); // min value
        ...
        if (selectedBoxes_array[i].includes("emoji")) {
          // contem emoji
          filtered.push(4); // ID
      ...
      for (let s = 0; s < toShow.length; s++) {
        for (let i = 0; i < content.length; i++) {
          if (toShow[s][0] == 3) {
            ...
            if (aux != objLength - 1) {
              if (
                content[i][1] < toShow[s][1] ||
                content[i][1] > toShow[s][2]
              ) {
                fits = false;
              }
              row.push(content[i]);
        ...
        <div className="popup-overlay">
          <div className="popupWithLink">
            <button class="parentBtn">
              <a
                class="popupLink"
                href={`/userIapDetails/${popup_userID}/${iapID}`}
              >
                {/*{`/userObjectiveDetails/${popup_userID}/${popup_objID}`} >*/}
              ...
              (
                ...
                <td>
                  <div className="td42">
                    ...
                    >
                      <div
                        className={`progress-bar variant-bg${element[3]}`}
                        style={{
                          width: element[1] < 10 ? "20px" : `${element[1]}%`,
                          backgroundColor:
                            element[3] == "success"
                              ? "#3cb371"
                              : element[3] == "warning"
          ...
          <thead>
            <tr>
              ...
              {objectives.map(
                (
                  objective,
                  index //props.state.objectives -> objectives
                ) => (
                  <th key={index}>
                    <Link
                      to={`/objectiveDetailsPage/${objectives_ids[index]}`}
                      style={{ color: "black" }}
                    >
                      {objective}
                    </Link>
...
class DashBoard extends React.Component {
  ...
  componentDidMount() {
    ...
    axios
      .get("/objectives") // Assuming the server is running on the same host and port as your React app
      .then((response) => {
        objectives = []; // clear
        objectives_ids = [];

        for (var i = 0; i < response.data.length; i++) {
          if (response.data[i].iap_id == iapID) {
            // objectivo corresponde à IAP que etsamos a analisar
