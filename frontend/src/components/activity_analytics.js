...
class ActivityAnalytics extends Component {
    ...
    fetch(process.env.REACT_APP_BACKEND + "/activity/" + this.props.id)
      ...
      .then((data) => {
        ...
        if (this.state.activity.analytics) {
          ...
          fetch(
            "https://inventivetraining.herokuapp.com/api/analyticsGetDocRoute",
            requestOptions
          )
            .then(async (response) => {
              ...
              if (!response.ok) {
                ...
                console.log(
                  "Error getting provider analytics: " + JSON.stringify(error)
                );
  ...
  render() {
    return (
      <div>
        <center>
          <h1>Analytics for "{this.state.activity.name}" activity</h1>
        </center>
        <br />
        <br />
        ...
        <Container fluid="xl">
          <h3>
            Analytics JSON: <a href={this.state.activity.analytics}>Download</a>
          </h3>
          <br />

          {this.state.analytics &&
            this.state.analytics.map((student) => (
              <div key={student.inveniraStdID}>
                <h4>Student ID - {student.inveniraStdID}</h4>
                <h5>
                  Qualitative Analytics -{" "}
                  <a href={student.qualAnalyticsURL}>Check on provider</a>
                </h5>
                <Table striped bordered hover>
                  ...
                  <tbody>
                    {student.quantAnalytics &&
                      student.quantAnalytics.map((analytic, index) => (
                        <tr key={index}>
                          <td>{index}</td>
                          <td>{analytic.name}</td>
