...
const EditIAP = () => {
  ...
  const onSaveObjective = (updatedObjective) => {
    ...
    const index = objectives.findIndex(
      (objective) => objective === editingObjective
    );
  ...
  const handleCreate = () => {
    ...
    axios
      .get("http://localhost:8000/objectives")
      .then((response) => {
  ...
  const postIAP = () => {
    ...
    axios
      .post("http://localhost:8000/iaps", data)
      .then((response) => {
  ...
  const postObjectives = (iapId) => {
    ...
    objectives.map((objective, index) => {
      if (objective.ObjectiveName !== "") {
        ...
        axios
          .post("http://localhost:8000/objectives", data)
          .then((response) => {
            console.log(response);
  ...
  const postObjectivesAnalytics = (row, id) => {
    ...
    axios
      .post("http://localhost:8000/objectivesanalytics", data)
      .then((response) => {
        console.log(response);
      })
      .catch((error) => {
  ...
  const postFlags = async (objective_id) => {
    try {
      // Replace with your own logic to retrieve objectives
          ...
          for (const key of keys) {
            ...
            for (let i = 0; i < emojiValues.length; i++) {
              ...
              const response2 = await axios.post(
                "http://localhost:8000/flagsobjectives",
                data2
              );
  ...
  const postScores = (objective_id, rowId) => {
    ...
    numbers.map((i) => {
      ...
      axios
        .post("http://localhost:8000/scores", data)
        .then((response) => {
          console.log(response);
        })
        .catch((error) => {
  ...
  return (
    <div className="container">
      <div className="title">
        <h1>
          <input
            className="editable-titles"
            type="text"
            value={iapName}
            onChange={handleIapNameChange}
          />
        </h1>
      ...
      <div className="bottom-section">
        <div
          ...
          style={{ display: showObjective ? "block" : "none" }}
        >
                  ...
                  <Table
                    activities={
                      editingObjective === null
                        ? selectedActivities
                        : editingObjective.selectedActivities
                    }
        ...
        <button
          className="set-objective-button"
          id="button-5"
