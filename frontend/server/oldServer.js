...
const db = new sqlite3.Database("../../database/database.db"); // Replace with a file path to create a persistent database
...
db.serialize(() => {
  ...
  // db.run(
  //   `INSERT INTO activities (id, acid, name, description) VALUES (1, '64a3fa101345d4b792686a56' ,'Group Roles', 'Simple Activity for directing students to choose a role')`
  // );
  // db.run(
  //   `INSERT INTO activities (id, acid, name, description) VALUES (2, '64a7420fd0ea814cd4a33568', 'Youtube Video', 'Link to a youtube video' )`
  // );

  // // ------------------------------
  // // IAP
  // db.run(
  //   `INSERT INTO iaps (id, name, description) VALUES (1, 'Group Roles & Youtube Video', 'Form Group Roles and watch a Youtube Video')`
  // );

  // // Obj 1
  // db.run(
  //   `INSERT INTO objectives (id, iap_id, name) VALUES (1, 1, 'Form Group Roles')`
  // );
  ...
  // // Analytics 1
  // db.run(
  //   `INSERT INTO analytics (id, activity_id, name, api_link, type) VALUES (1, 1, 'Form Groups', 'http://localhost:3000/', 'Qualitative')`
  // );
  // db.run(
  //   "INSERT INTO objectivesanalytics (objective_id, analytics_id, weight) VALUES (1, 1, 100)"
  // );

  // db.run(
  //   `INSERT INTO flags (id, analytics_id, min_value, max_value, flag) VALUES (1, 1, 0, 50, '😵')`
  // );
  // db.run(
  //   `INSERT INTO flags (id, analytics_id, min_value, max_value, flag) VALUES (2, 1, 51, 100, '🥳')`
  // );

  // // Obj 2
  // db.run(
  //   `INSERT INTO objectives (id, iap_id, name) VALUES (2, 1, 'Watch a youtube video')`
  // );
  ...
  // // Analytics 2
  // db.run(
  //   `INSERT INTO analytics (id, activity_id, name, api_link, type) VALUES (2, 2, 'Watch Video', 'http://localhost:3456/youtube', 'Qualitative')`
  // );
  // db.run(
  //   "INSERT INTO objectivesanalytics (objective_id, analytics_id, weight) VALUES (2, 2, 100)"
  // );

  // db.run(
  //   `INSERT INTO flags (id, analytics_id, min_value, max_value, flag) VALUES (3, 2, 0, 50, '😵')`
  // );
  // db.run(
  //   `INSERT INTO flags (id, analytics_id, min_value, max_value, flag) VALUES (4, 2, 51, 100, '🥳')`
  // );
});
...
app.post("/objectives", (req, res) => {
  ...
  db.run(
    "INSERT INTO objectives (iap_id, name) VALUES (?, ?)",
    [iap_id, name],
    function (err) {
      if (err) {
        res.status(500).json({ error: err.message });
...
app.post("/objectivesanalytics", (req, res) => {
  ...
  db.run(
    "INSERT INTO objectivesanalytics (objective_id, analytics_id, weight) VALUES (?, ?, ?)",
    [objective_id, analytics_id, weight],
    function (err) {
      if (err) {
        res.status(500).json({ error: err.message });
      } else {
...
app.post("/flagsobjectives", (req, res) => {
  ...
  db.run(
    "INSERT INTO flagsobjectives (flag_id, objective_id) VALUES (?, ?)",
    [flag_id, objective_id],
    function (err) {
      if (err) {
        res.status(500).json({ error: err.message });
      } else {
...
app.get("/analytics", (req, res) => {
  ...
  if (activity_id) {
    const activityIds = Array.isArray(activity_id)
      ? activity_id
      : [activity_id];
...
app.post("/analytics", (req, res) => {
  ...
  db.run(
    "INSERT INTO analytics (activity_id, name, api_link, type) VALUES (?, ?, ?, ?)",
    [activity_id, name, api_link, type],
    function (err) {
      if (err) {
        res.status(500).json({ error: err.message });
      } else {
...
app.post("/flags", (req, res) => {
  ...
  db.run(
    "INSERT INTO flags (analytics_id, min_value, max_value, flag) VALUES (?, ?, ?, ?)",
    [analytics_id, min_value, max_value, flag],
    function (err) {
      if (err) {
        res.status(500).json({ error: err.message });
      } else {
...
app.post("/scores", (req, res) => {
  ...
  db.run(
    "INSERT INTO scores (user_id, analytics_id, objective_id, score) VALUES (?, ?, ?, ?)",
    [user_id, analytics_id, objective_id, score],
    function (err) {
      if (err) {
        res.status(500).json({ error: err.message });
      } else {
    ...
    if (err) {
      ...
      if (row) {
        ...
        if (score > row.score) {
          ...
          res.json({
            message:
              "O score existente é maior ou igual. Nenhuma atualização necessária.",
          });
        }
      } else {
